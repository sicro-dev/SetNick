    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("nick")) {
            if (args.length == 1 && args[0].equalsIgnoreCase("random")) {
                ...
                if (nicksDisponiveis.isEmpty()) {
                    player.sendMessage("Todos os nicks disponíveis já foram utilizados.");
                    return true;
                }
                ...
                player.sendMessage("Seu nick foi alterado para " + novoNick);
                return true;
            } else if (args.length != 1) {
                player.sendMessage("Uso correto: /nick [novo-nick]");
                return true;
            }
